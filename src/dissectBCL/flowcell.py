...
from dissectBCL.misc import P5Seriesret, umlautDestroyer
...
class flowCellClass:
    ...
    def filesExist(self):
        ...
        for f in [
            ...
            if not f.exists():
                ...
                mailHome(
                    self.name, f"{f} does not exist. dissectBCL crashed.", self.config
                )
                sys.exit(1)
    ...
    def parseRunInfoAviti(self):
        ...
        if "R2" in cycles_dict:
            seqRecipe["Read2"] = ["Y", cycles_dict["R2"]]
        if "I1" in cycles_dict:
            seqRecipe["Index1"] = ["I", cycles_dict["I1"]]
        if "I2" in cycles_dict:
            seqRecipe["Index2"] = ["I", cycles_dict["I2"]]
        lanes = 2 if run_params["AnalysisLanes"] == "1+2" else 1
    ...
    def validateRunCompletion(self):
        ...
        if self.sequencer == "Miseq":
            ...
            # no RunCompletionStatus.xml in novaseq, assume success.
    ...
    def prepConvert(self):
        ...
        """
        logging.info(
            "Demux - prepConvert - determine masking, indices, paired ends, and other options"
        )
        for outputFolder in self.sampleSheet.ssDic:
            ...
            (
                ...
                minP5,
                minP7,
            ) = detMask(self.seqRecipe, ss, outputFolder, self.sequencer)
        ...
        if self.successfulrun != "SuccessfullyCompleted":
            ...
            for outLane in self.sampleSheet.ssDic:
                ...
                if not demuxOut.exists():
                    ...
                    writeDemuxSheet(demuxOut, _ssDic, self.sampleSheet.laneSplitStatus)
                else:
                    logging.warning(
                    ...
                    if exitcode == 0:
                        ...
                        if self.sequencer == "MiSeq":
                            if evalMiSeqP5(
                                ...
                                _stdout, _stderr = bclRunner.communicate()
                                logging.info(
                                    f"Demux - bclConvert P5fix exit {exitcode}"
                                )
        ...
        if self.successfulrun != "SuccessfullyCompleted":
            ...
            for outLane in self.sampleSheet.ssDic:
                ...
                if not demuxOut.exists():
                    ...
                    writeDemuxSheetAviti(
                        demuxOut, _ssDic, self.sampleSheet.laneSplitStatus
                    )
                else:
                    logging.warning(
                ...
                _ssDic["sampleSheet"] = parseStats(
                    outputFolder, _ssDic["sampleSheet"], mode="aviti"
                )
        ...
        for outLane in self.sampleSheet.ssDic:
            ...
            for project in projects:
                if not renameFlag.exists():
                    logging.info(f"Postmux - renaming {outLane}")
                    if self.sequencer == "aviti":
                        # Aviti mode, rename project folder
                        renameProject(
                            laneFolder / "Samples" / project,
                            df,
                            self.sampleSheet.laneSplitStatus,
                        )
                    else:
                        renameProject(
                            laneFolder / project, df, self.sampleSheet.laneSplitStatus
                        )
    ...
    def fakenews(self):
        ...
        for outLane in self.sampleSheet.ssDic:
            ...
            self.exitStats[outLane]["pushParkour"] = pushParkour(
                self.flowcellID,
                self.sampleSheet,
                self.config,
                self.bclPath,
    ...
    def __init__(self, name, bclPath, logFile, config, sequencer, forceLaneSplit):
        ...
        if sequencer == "illumina":
            ...
            self.seqRecipe, self.lanes, self.instrument, self.flowcellID = (
                self.parseRunInfo()
            )
        else:
            # Aviti mode.
            ...
            self.seqRecipe, self.lanes, self.instrument, _fid = (
                self.parseRunInfoAviti()
            )  # discard flowcell ID until it can be used consistently with Parkour
    ...
    def asdict(self):
        return {
            ...
            if hasattr(self, "runCompletionStatus") and self.runCompletionStatus
            else "",
...
class sampleSheetClass:
    ...
    - At initiation stage parkour is queried.
    """

    def decideSplit(self, aviti):
        """
         Do we need to split per lane ?
        Lane splitting = preferred:
        ...
        for _s in samples:
            if (
                len(
                    list(
                        self.fullSS[self.fullSS[sample_colname] == _s][
                            lane_colname
                        ].unique()
                    )
                )
                ...
                logging.info("No lane splitting: >= 1 sample in multiple lanes.")
        ...
        for project in projects:
            if (
                len(
                    list(
                        self.fullSS[self.fullSS[project_colname] == project][
                            lane_colname
                        ].unique()
                    )
                )
                ...
                logging.info("No lane splitting: >= 1 project in multiple lanes.")
                laneSplitStatus = False
        # Don't split if 1 lane in ss, multiple in runInfo
        if len(list(self.fullSS[lane_colname].unique())) < self.runInfoLanes:
            logging.info(
                f"No lane splitting: 1 lane listed, {self.runInfoLanes} found."
            )
            laneSplitStatus = False
        # Make sure:
        # if laneSplitStatus = False:
        # No samples can clash at all!
        if lane_colname in list(self.fullSS.columns) and not laneSplitStatus:
            if index1_colname in list(self.fullSS.columns) and index2_colname in list(
                self.fullSS.columns
            ):
        ...
        logging.info(f"decide_lanesplit returns {laneSplitStatus}")
    ...
    def parseSS(self, parkourDF) -> dict:
        ...
        ssdf["Sample_Name"] = ssdf["Sample_Name"].apply(lambda x: umlautDestroyer(x))
            ...
            for lane in range(1, self.runInfoLanes + 1, 1):
                ...
                if not parkourDF.empty:
                    ...
                    if "-" in self.flowcell:
                        ...
                        samples, not necessarily those sequenced.
                        """
                        newReqDepth = (
                            20 / len(list(mergeDF["Sample_Name"].unique())) * 1000000
                        )
                        newReqDepth = round(newReqDepth, 0)
                        mergeDF["reqDepth"] = newReqDepth
                        logging.debug(
                            f"miSEQ detected, override seqdepth: {newReqDepth}"
                        )
    ...
    def parseSS_aviti(self, parkourDF) -> dict:
        ...
        if "Description" not in ssdf.columns:
            logging.info(
                "Description column not found in RunManifest. Adding an empty column."
            )
...
class drHouseClass:
    ...
    def prepMail(self):
        ...
        for key in self.exitStats:
            if key in ["premux", "demux", "postmux"]:
        ...
        msg = (
            ...
            + "<h3>Samples</h3>"
            + tabulate(tableCont, tableHead, tablefmt="html", disable_numparse=True)
        )
