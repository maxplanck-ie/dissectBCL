    ...
    for i, ix_list in enumerate((P7s, P5s)):
        ...
        if not ix_list.empty and not ix_list.isnull().all():
            for comb in combinations(ix_list, 2):
                ...
                if sequencer == "aviti":
                    barcode_mm = f"I{i + 1}MismatchThreshold"
                else:
                    barcode_mm = f"BarcodeMismatchesIndex{i + 1}"
...
def detMask(seqRecipe, sampleSheetDF, outputFolder, sequencer):
    ...
    """
    logging.info(f"determine masking for {outputFolder}")
    logging.info(f"masking for seqRecipe {seqRecipe}")
    ...
    if sequencer == "aviti":
        mask = {"R1FastQMask": "", "R2FastQMask": "", "I1Mask": "", "I2Mask": ""}
    else:
        mask = []
    dualIx = False
    PE = False
    ...
    if not any(sr.startswith("Index") for sr in seqRecipe):
        if sequencer == "aviti":
            mask["R1FastQMask"] = joinLis(seqRecipe["Read1"])
            if not dualIx and P5seq:
                mask["I2Mask"] = f"N{recipeP5}"
            if PE:
    ...
    if "indexType" in list(sampleSheetDF.columns):
        ...
        # Nugen Ovation SOLO
        if any(
            sampleSheetDF["indexType"]
            .dropna()
            .str.contains("NuGEN Ovation SoLo RNA-Seq System")
        ):
            logging.info(f"NuGEN Ovation SoLo found for {outputFolder}.")
            # Read 1
            if sequencer == "aviti":
                mask["R1FastQMask"] = joinLis(seqRecipe["Read1"])
            else:
            ...
            if recipeP7 - minP7 > 0:
                if sequencer == "aviti":
                    mask["I1Mask"] = f"Y{minP7}N{recipeP7 - minP7}"
                else:
                    mask.append(f"I{minP7}U{recipeP7 - minP7}")
            else:
                logging.warning("NuGEN Ovation solo Index read length == P7!")
                if sequencer == "aviti":
                    mask["I1Mask"] = f"Y{minP7}"
                else:
                    mask.append(f"I{minP7}")
            # Index 2
            if dualIx:
                if sequencer == "aviti":
            ...
            if sequencer == "aviti":
                ...
                return ";".join(mask), dualIx, PE, convertOpts, None, None
        # scATAC
        elif any(sampleSheetDF["Description"].dropna().str.contains("|".join(scATACl))):
            logging.info(f"scATAC seq found for {outputFolder}")
            # Read 1
            if sequencer == "aviti":
                mask["R1FastQMask"] = joinLis(seqRecipe["Read1"])
            else:
            ...
            if "Index2" in seqRecipe:
                if sequencer == "aviti":
                    if np.isnan(recipeP5):
                        recipeP5 = seqRecipe["Index2"][1]
                    mask["I2Mask"] = f"I2:N{recipeP5}"
                    mask["UmiMask"] = f"I2:Y{recipeP5}"
                    mask["I1FastQ"] = True
                    mask["I2FastQ"] = True
                    mask["UmiFastQ"] = True
                else:
                    mask.append(f"U{recipeP5}")
            if dualIx:
                logging.warning(
            ...
            logging.info(f"{outputFolder} is not special. Setting default mask")
            # Read 1
            if sequencer == "aviti":
                mask["R1FastQMask"] = joinLis(seqRecipe["Read1"])
            else:
...
def writeDemuxSheet(demuxOut, ssDic, laneSplitStatus):
    ...
    if "mismatch" in ssDic:
        for i in (1, 2):
            bc_str = f"BarcodeMismatchesIndex{i}"
    ...
    if ssDic["dualIx"]:
        ...
        if laneSplitStatus:
            ...
            for _index, row in ssdf_towrite.iterrows():
                demuxSheetLines.append(
                    joinLis(
                        [
                            row["Lane"],
        ...
        if laneSplitStatus:
            ...
            for _index, row in ssdf_towrite.iterrows():
                demuxSheetLines.append(
                    joinLis(
                        [row["Sample_ID"], row["index"], row["Sample_Project"]],
                        joinStr=",",
                    )
                )
...
def writeDemuxSheetAviti(demuxOut, ssDic, laneSplitStatus):
    ...
    if "mismatch" in ssDic:
        for i in (1, 2):
            bc_str = f"I{i}MismatchThreshold"
            if i == 2 and not ssDic["dualIx"]:
                continue
            if bc_str in ssDic["mismatch"]:
                demuxSheetLines.append(
    ...
    if ssDic["dualIx"]:
        ...
        demuxSheetLines.append("SampleName,Index1,Index2,Lane,Project")
        for _index, row in ssdf_towrite.iterrows():
            demuxSheetLines.append(
                joinLis(
                    [
        ...
        demuxSheetLines.append("[SAMPLES],,,,")
        demuxSheetLines.append("SampleName,Index1,Lane,Project")
        for _index, row in ssdf_towrite.iterrows():
            demuxSheetLines.append(
                joinLis(
                    [row["SampleName"], row["Index1"], row["Lane"], row["Project"]],
                    joinStr=",",
                )
            )
...
def readDemuxSheet(demuxSheet, what="all"):
    ...
    with open(demuxSheet) as f:
        ...
        for line in f:
            ...
            if line.startswith("OverrideCycles"):
                mask = line.replace("OverrideCycles", "").replace(",", "")
            if sampleStatus:
    ...
    df = pd.DataFrame(nesLis[1:], columns=nesLis[0])
    dualIx = "index2" in list(df.columns)
...
def parseStats(outputFolder, ssdf, mode="illumina") -> pd.DataFrame:
    if mode == "illumina":
        ...
        QCmetDic = {}
        for _index, row in QCdf.iterrows():
        ...
        muxDic = {}
        for _index, row in muxdf.iterrows():
            sampleID = row["SampleID"]
        ...
        for ID in QCmetDic:
            ...
            for read in QCmetDic[ID]:
                QCstr += f"{read}:{round(QCmetDic[ID][read][0], 2)},"
                Perc30str += f"{read}:{round(QCmetDic[ID][read][1], 2)},"
            QCstr = QCstr[:-1]
        ...
        for samplestat in (outputFolder).rglob("*_stats.json"):
            ...
            match len(stats["Reads"]):
                ...
                case 2:
                    # paired-end
                    ssdf.loc[ssdf["Sample_ID"] == sampleID, "meanQ"] = (
                        f"1:{_qsm},2:{_qsm}"
                    )
                    ssdf.loc[ssdf["Sample_ID"] == sampleID, "percQ30"] = (
                        f"1:{_q30},2:{_q30}"
                    )
                    ssdf.loc[ssdf["Sample_ID"] == sampleID, "gotDepth"] = int(_depth)
        return ssdf
...
def compareDemuxSheet(ssDic, demuxSheet):
    ...
    """
    man_mask, man_df, man_dualIx, man_mmdic = readDemuxSheet(demuxSheet)
    # mmdic
    ...
    # sampleSheet
    ssDic["sampleSheet"] = matchingSheets(ssDic["sampleSheet"], man_df)
    # P5RC
    ssDic["P5RC"] = False
...
def matchingSheets(autodf, mandf):
    ...
    if len(autodf.index) != len(mandf.index):
        logging.warning("Demux - number of samples changed in overwritten demuxSheet !")
    ...
    for index, row in mandf.iterrows():
        ...
        if dualIx:
            ...
            if autodf.loc[pdIx, "index2"].values != index2:
                ...
                logging.debug(
                    f"Demux - Changing P5 {oriP5} to {index2} for {sample_ID}"
                )
...
def evalMiSeqP5(outPath, dualIx):
    ...
    if numLowreadSamples / totalSamples != 1:
        return False
    logging.warning(
    ...
    shutil.move(demuxSheetPath, demuxSheetPath.with_suffix(".bak"))
