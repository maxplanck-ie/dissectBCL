...
def getConf(configfile, quickload=False):
    ...
    if not quickload:
        ...
        # kraken2 version
        p = sp.run(["kraken2", "--version"], capture_output=True)
        kraken2 = p.stdout.decode().splitlines()[0].split(" ")[2]
        # clumpifyVer
        p = sp.run(["clumpify.sh", "--version"], capture_output=True)
        clumpify = "Not found"
        ...
        if not Path(config["software"]["fastqc_adapters"]).exists():
            sys.exit(
                f"Fastqc adapters file under {config['software']['fastqc_adapters']} not found. Please check your config file."
            )
    return config
...
def getNewFlowCell(
    ...
    if fPath:
        ...
        if not any(outBaseDir.glob(f"{flowcellName}*/communication.done")):
            ...
            print(
                f"[red]{flowcellName} exists with a communication.done flag already.[/red]"
            )
            sys.exit()
...
def parseRunInfo(runInfo):
    ...
    for i in root.iter():
        if i.tag == "Read":
            readType = "Index" if i.attrib["IsIndexedRead"] == "Y" else "Read"
...
def lenMask(recipe, minl, aviti):
    ...
    if recipe - minl > 0:
        return (
            f"Y{int(minl)}N{int(recipe - minl)}"
            if aviti
            else f"I{int(minl)}N{int(recipe - minl)}"
        )
    else:
...
def krakenfqs(IDdir):
    ...
    for fq in sorted(Path(IDdir).glob("*fastq.gz")):
        if fq.name.endswith("_R1.fastq.gz") or fq.name.endswith("_R2.fastq.gz"):
            fqFiles.append(fq)

    krakRep = str(fqFiles[0]).replace("_R1.fastq.gz", "") + ".rep"
    krakRep = krakRep.replace(
        "Project_", "FASTQC_Project_"
    )  # output to fastqc folder, not project.
    if len(fqFiles) == 1:
        return (krakRep, [str(fqFiles[0])])
    elif len(fqFiles) == 2:
        return (krakRep, ["--paired", str(fqFiles[0]), str(fqFiles[1])])
...
def multiQC_yaml(flowcell, project, laneFolder):
    ...
    # seqreport Data
    seqreportData = ""
    for _index, row in ssdf.iterrows():
        if seqreportData == "":
            ...
            seqreportData += f"\tSample ID\tLane\t{meanQ_headers}\t{percq30_headers}\n"
            seqreportData += "{}\t{}\t{}\t{}\t{}\n".format(
                row["Sample_Name"],
                row["Sample_ID"],
                "L" + str(row["Lane"]),
    ...
    for _index, row in ssdf.iterrows():
        if indexreportData == "":
            indexreportData += f"\tSample ID\t{retBCstr(row, returnHeader=True)}\t{retIxtype(row, returnHeader=True)}\n"
        indexreportData += "{}\t{}\t{}\t{}\n".format(
            row["Sample_Name"], row["Sample_ID"], retBCstr(row), retIxtype(row)
        )
    ...
    # Organisms
    orgs = ", ".join(list(ssdf["Organism"].str[0].fillna("None").unique()))
    # Resequencing runs are screwed up (e.g. don't contain the samples)
    # switch total requested to NA
    try:
        sumReqRound = str(round((ssdf["reqDepth"].sum()) / 1000000, 0))
    except TypeError:
        sumReqRound = "NA"
    ...
    mqcyml = {
        ...
        "report_header_info": [
            ...
            {"Requested reads": sumReqRound},
            {
                "Received reads": str(
                    round(
                        (ssdf["gotDepth"].replace("NA", np.nan).dropna().sum())
                        / 1000000,
                        0,
                    )
                )
...
def matchOptdupsReqs(optDups, ssdf):
    ...
    for lis in optDups:
        sampleID = lis[1]
        sampleName = lis[2]
        req = ssdf[ssdf["Sample_ID"] == sampleID]["reqDepth"].values
        got = ssdf[ssdf["Sample_ID"] == sampleID]["gotDepth"].values
