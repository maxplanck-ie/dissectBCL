...
def dumpnode(taxmap, ofile):
    with open(ofile, "w") as f:
        for taxon in taxmap:
            f.write(
                f"{taxmap[taxon][0]}\t|\t{taxmap[taxon][1]}\t|\t{taxmap[taxon][2]}\t|\t-\t|\n"
            )
...
def dumpname(taxmap, ofile):
    with open(ofile, "w") as f:
        for taxon in taxmap:
            f.write(f"{taxmap[taxon][0]}\t|\t{taxon}\t|\t-\t|\tscientific name\t|\n")
...
def mask(mtupe, libdir):
    ...
    """
    print(f"Building blast db {mtupe}")
    bldb = [
        "makeblastdb",
        "-dbtype",
        "nucl",
    ...
    with open("tmpdb_bln") as f:
        for line in f:
            ...
            if int(lis[8]) > int(lis[9]):
                bed.append([lis[1], lis[9], lis[8]])
            else:
                bed.append([lis[1], lis[8], lis[9]])
...
def main(contaminome, outputdir, threads):
    ...
    librarydir = os.path.join(contaminomedir, "library")
    os.mkdir(librarydir)
    # read up yaml file.
    ...
    for domain in genomes:
        for sp in genomes[domain]:
            print(f"Fetching - {sp}")
            if genomes[domain][sp]["vulgarname"] in list(ignore_chrs.keys()):
                ignoring = ignore_chrs[genomes[domain][sp]["vulgarname"]]
            else:
                ignoring = []
            ofile = os.path.join(librarydir, genomes[domain][sp]["vulgarname"] + ".fna")
            if not os.path.exists(ofile):
                with (
                    urllib.request.urlopen(genomes[domain][sp]["URL"]) as gzip_genome,
                    gzip.GzipFile(fileobj=gzip_genome) as f,
                ):
                ...
                with open(ofile, "w") as f:
                    for line in genome:
                        if line.startswith(">"):
                            ...
                            appendStatus = headStr not in ignoring
                            if appendStatus:
                                headStr += "|kraken:taxid|"
                                headStr += str(genomes[domain][sp]["taxid"])
                                f.write(">" + headStr + "\n")
                                seq2acc[headStr] = genomes[domain][sp]["taxid"]
                        else:
                            if appendStatus:
                                f.write(line.strip() + "\n")
